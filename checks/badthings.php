		...
		foreach ( $php_files as $php_key => $phpfile ) {
			...
			checkcount();
				if ( preg_match( $key, $phpfile, $matches ) ) {
					...
					$this->error[] = sprintf('<span class="tc-lead tc-warning">'. __( 'WARNING', 'theme-check' ) . '</span>: ' . __( 'Found %1$s in the file %2$s. %3$s. %4$s', 'theme-check' ), '<strong>' . $error . '</strong>', '<strong>' . $php_key . '</strong>', $check, $grep );
					$ret = false;
				}
			}
		}
		...
		foreach ( $other_files as $php_key => $phpfile ) {
			foreach ( $checks as $key => $check ) {
				...
				if ( preg_match( $key, $phpfile, $matches ) ) {
					...
					$grep = tc_grep( $error, $php_key );
					$this->error[] = sprintf(__('<span class="tc-lead tc-warning">WARNING</span>: Found <strong>%1$s</strong> in the file <strong>%2$s</strong>. %3$s.%4$s', 'theme-check'), $error, $php_key, $check, $grep);
					$ret = false;
				}
			}
		}
