		...
		foreach ( $php_files as $php_key => $phpfile ) {
			foreach ( $checks as $key => $check ) {
				...
				if ( preg_match( $key, $phpfile, $matches ) ) {
					...
					$this->error[] = sprintf('<span class="tc-lead tc-warning">'.__( 'WARNING', 'theme-check' ) . '</span>: <strong>%1$s</strong>. %2$s%3$s', $php_key, $check, $grep );
					$ret = false;
				}
			}
		}
			...
			foreach ( $checks as $key => $check ) {
				...
				if ( preg_match_all( $key, $phpfile, $matches ) ) {
					foreach ($matches[1] as $match) {
						...
						$this->error[] = sprintf('<span class="tc-lead tc-required">'.__( 'REQUIRED', 'theme-check' ) .'</span>: <strong>%1$s</strong>. %2$s%3$s', $php_key, $check, $grep);
						$ret = false;
					}
				}
			}
