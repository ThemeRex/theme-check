		...
		foreach ( $php_files as $file_path => $file_content ) {
			...
			if ( preg_match_all( '/\$wp_customize->add_setting\(([^;]+)/', $file_content, $matches ) ) {
				...
				foreach ( $matches[0] as $match ) {
					$error = ltrim( $match, '(' );
					$error = rtrim( $error, '(' );
					$grep = tc_grep_multiline( $error, $file_path );
					...
					if ( false === strpos( $match, 'sanitize_callback' ) && false === strpos( $match, 'sanitize_js_callback' ) ) {
						$this->error[] = '<span class="tc-lead tc-required">' . __('REQUIRED','theme-check') . '</span>: ' . sprintf(__( 'Found a Customizer setting in %1$s that did not have a sanitization callback function. Every call to the <strong>add_setting()</strong> method needs to have a sanitization callback function passed.%2$s', 'theme-check' ),
						...
						if ( preg_match( '/[\'"](?:sanitize_callback|sanitize_js_callback)[\'"]\s*=>\s*[\'"]\s*[\'"]/', $match ) ) {
							$this->error[] = '<span class="tc-lead tc-required">' . __('REQUIRED','theme-check') . '</span>: ' . sprintf(__( 'Found a Customizer setting in %1$s that had an empty value passed as sanitization callback. You need to pass a function name as sanitization callback.%2$s', 'theme-check' ),
									'<strong>' . $file_path . '</strong>',
									$grep);
							$ret = false;
						}
